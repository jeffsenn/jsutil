(function (factory) {
  ...
  function orderedJSON(o) {
    if (Array.isArray(o)) {
      ...
      return "{" + Object.entries(o).map(function (a, b) {
        return '"' + a + '":' + orderedJSON(b);
      }).sort().join(",") + "}";
    } else {
  ...
  function shallowCopy(a) {
    return Array.isArray(a) ? a.slice() : typeof a === 'object' ? { ...a
    } : a;
  ...
  function deepEdit(obj, path, value, i) {
    ...
    if (val === undefined || val === null) delete obj[path[i]];else obj[path[i]] = val;
    return obj;
  ...
  const ErrTok = deepFreeze({
    "": ["ErrTok"]
  });
  const Null = deepFreeze({
    "": ["Null"]
  }); //NOT js 'null'
  ...
  class DateValue {
    constructor(data) {
      ...
      if (data === undefined) {
        ...
        if (isNaN(d)) throw Error("not convertable to date"); //big question: should this be ISO format?
        //data = d.toISOString()
      }
    ...
    async function doEval(node) {
      switch (node.type) {
        case 'MemberExpression':
          const name = node.computed ? doEval(node.property) : node.property.type === 'Literal' ? node.property.value : node.property.type === 'Identifier' ? node.property.name : null;
  ...
  async function asyncEvaluate(e, vars, literal) {
    async function doEval(node) {
      switch (node.type) {
        case 'MemberExpression':
          const name = node.computed ? await doEval(node.property) : node.property.type === 'Literal' ? node.property.value : node.property.type === 'Identifier' ? node.property.name : null;
          if (!name) throw "bad member expr: " + JSON.stringify(node.property);
  ...
  async function grql(startAt, query, options) {
    ...
    const findReference = options && options['findReference']; //make a mapping table from column names to row value

    function colVars(row, colName) {
      return Object.keys(colName).reduce((ret, k) => {
        ret[k] = row[colName[k]];
        return ret;
      }, {});
    ...
    function extendRows(rows, follow, allowNull = false) {
      ...
      for (let i = 0; i < rows.length; i++) {
        const row = rows[i]; //take last element out of row and attempt to follow

        const val = row && row[row.length - 1];
        const valToDeref = isReference(val) ? findReference(val) : val; //what deRef value is now an array (of instances) -- we should extend all the rows....
    ...
    async function customEvalExpr(e, vars) {
      class ManyValued {
        ...
        followMemberJoin(n) {
          const ret = this.items.reduce((ret, i) => {
            const v = i[n];
      ...
      async function literalAccess(v, n, isRoot) {
        ...
        if (n === '') ; else if (v instanceof ManyValued) {
